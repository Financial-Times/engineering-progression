...
This short guide covers some key files and directories which you'll need to know about to edit the site. Site content follows our [tone and language guidelines](language.md).
  ...
  - The competencies and levels data is generated automatically, so that this data is always pinned at the latest released version on the website. Any changes to files in here will be overwritten. If you need to make changes to the way these files are generated, see `scripts/build-website-data.js`.
...
The local website will not have updated competencies data unless you generate the data manually. On CI and in production this is automated, but to build these parts of the site manually run the following once and then each time you make a data change:
